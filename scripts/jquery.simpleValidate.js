 ...
 *
 * Dodatek: dodal preverjanje pri weburl
 * Avtor: Samir Subasic, (http://www.frontend.si) 2011
 * 
 * @version 0.1
 *
 * Example usage:
...
;(function($) {
		...
		return this.each(function() {
			...
			$this.bind('submit', function(e) {
				...
				$this.find(':input.required').each(function() {
                    ...
                    } else if($input.hasClass('weburl')) {                        
					  if(!(/(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/.test(fieldValue))) {
					    errorMsg = (errorMsgType > -1 ) ? errorMsg = o.webUrlErrorText.replace('{label}',labelText) : errorMsg = o.webUrlErrorText;
					    hasError = true;
					  }
					}
	...
	$.fn.simpleValidate.defaults = {
		errorClass: 'error',
		errorText: 'Polje {label} mora biti izpolnjeno!',
		emailErrorText: 'Prosim pravilno vnesite {label}',
        webUrlErrorText: 'Prosim pravilno vnesite {label}',
		errorElement: 'strong',
		removeLabelChar: '*',
		inputErrorClass: '',
		completeCallback: '',
